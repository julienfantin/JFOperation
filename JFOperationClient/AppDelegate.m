...
//  JFOperationClient
//
//  Created by Julien Fantin on 20/09/12.
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions
{
    ...
    __block NSOperationQueue *q = self.queue;
    
    for (int i=0; i<10; i++) {
        
        JFOperation *operation = [JFOperation new];

        operation.isConcurrent = NO;
        
        operation.onStart = ^(JFOperation *o) {
            NSLog(@"%@ started", o);
        };
        ...
        operation.onFinish = ^(JFOperation *o) {
            NSLog(@"%@ finished", o);
        };
